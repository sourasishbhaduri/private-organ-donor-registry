...
export class Contract {
  ...
  constructor(...args_0) {
    ...
    this.circuits = {
      testCircuit: (...args_1) => {
        if (args_1.length !== 3) {
          throw new __compactRuntime.CompactError(`testCircuit: expected 3 arguments (as invoked from Typescript), received ${args_1.length}`);
        }
        const contextOrig_0 = args_1[0];
        const secret_0 = args_1[1];
        const age_0 = args_1[2];
        if (!(typeof(contextOrig_0) === 'object' && contextOrig_0.currentQueryContext != undefined)) {
          __compactRuntime.typeError('testCircuit',
                                     'argument 1 (as invoked from Typescript)',
                                     'test.compact line 7 char 1',
                                     'CircuitContext',
                                     contextOrig_0)
        }
        if (!(typeof(secret_0) === 'bigint' && secret_0 >= 0n && secret_0 <= 4294967295n)) {
          __compactRuntime.typeError('testCircuit',
        ...
        const partialProofData = {
          input: {
            value: _descriptor_1.toValue(secret_0).concat(_descriptor_1.toValue(age_0)),
        ...
        const result_0 = this._testCircuit_0(context,
                                             partialProofData,
                                             secret_0,
                                             age_0);
    ...
    this.impureCircuits = { testCircuit: this.circuits.testCircuit };
    this.provableCircuits = { testCircuit: this.circuits.testCircuit };
  }
  initialState(...args_0) {
    if (args_0.length !== 1) {
    ...
    state_0.setOperation('testCircuit', new __compactRuntime.ContractOperation());
  ...
  _testCircuit_0(context, partialProofData, secret_0, age_0) {
    __compactRuntime.assert(age_0 >= 18n, 'Age must be at least 18');
    const tmp_0 = 1n;
    __compactRuntime.queryLedgerState(context,
                                      partialProofData,
                                      [
